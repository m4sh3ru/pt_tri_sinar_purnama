...
use Session, Auth, Helper ,Validator;
...
class KerusakanController extends Controller
{
	...
	public function index(Request $req)
	{
		...
		if($req->input('start_date'))
		{
			...
			$q = Kerusakan::whereBetween('created_at',[date('Y-m-d 00:00:00',strtotime($start)), date('Y-m-d 23:59:59',strtotime($end))])->orderBy('id', 'DESC')->paginate(15);
		}else{
	...
	public function store(Request $request)
	{
		if(Auth::check())
		{
			...
			$rules = ['nama'=>'required|mimes:pdf,ppt,doc,docx,pptx'];
			$validator = Validator::make(['image'=>$file], $rules);

			if($validator->fails()){
				//**upload file processing **//
				$destination = public_path() . '/media/screenshot/';
				
				if($request->hasFile('image'))
				{
					$filename = $request->file('image')->getClientOriginalName();
					$extension = $request->file('image')->getClientOriginalExtension();
					//uploading file or photo to the destination folder
